var imgBase64 = localStorage['image'];
...
class Screen {
    ...
    maskShow(x, y, _x, _y) {
        ...
        this.canvasMask.style.left = (_x < x ? _x : x)-1 + 'px';
        this.canvasMask.style.top = (_y < y ? _y : y)-1 + 'px';
    }
    ...
    dragEvent(x, y) {
        ...
        // 重新绘制选区
        // console.log.apply(console,['重新绘制选区']);
        this.createRect(_x, _y, x0+1, y0+1, true);
        // 隐藏工具栏
        this.hideTool();
    ...
    save() {
        ...
        remote.dialog.showSaveDialog({
            title:'保存截图',
            defaultPath:'~/IYou-'+new Date().getTime()+'.png',
            filters: [{ name: 'Images', extensions: ['jpg', 'png', 'gif'] }],
        }, function(pathStr) {
            if (pathStr) {
                fs.writeFile(pathStr, new Buffer(imgData.replace(/^data:image\/\w+;base64,/, ''), 'base64'), function(err) {
                    if(err){
                        console.log('保存失败')
                    }else{
                        that.close();
...
new Screen('canvas', 'canvasMask', imgBase64);
